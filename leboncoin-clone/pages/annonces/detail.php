...
<div style="max-width: 900px; margin: 0 auto;">
    <a href="<?= url('/pages/annonces/liste.php') ?>" style="color: var(--accent); text-decoration: none; margin-bottom: 1rem; display: inline-block;">← Retour aux annonces</a>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; background: var(--bg-primary); padding: 2rem; border-radius: var(--radius); border: 1px solid var(--border);">
        <div>
            <img src="<?= url('/assets/uploads/' . e($images[0] ?? 'placeholder.jpg')) ?>" alt="<?= e($annonce['titre']) ?>" style="width: 100%; border-radius: var(--radius); object-fit: cover; max-height: 400px;">
            <?php if (count($images) > 1): ?>
                <div style="display: flex; gap: 0.5rem; margin-top: 1rem; overflow-x: auto;">
                    <?php foreach ($images as $img): ?>
                        <img src="<?= url('/assets/uploads/' . e($img)) ?>" style="width: 80px; height: 80px; object-fit: cover; border-radius: 4px; cursor: pointer; border: 1px solid var(--border);">
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div>
            ...
            <div style="font-size: 2rem; font-weight: 800; color: var(--accent); margin-bottom: 1.5rem;"><?= number_format((float)$annonce['prix'], 2, ',', ' ') ?> €</div>
...
<div style="background: var(--bg-secondary); padding: 1.5rem; border-radius: var(--radius); margin-bottom: 1.5rem;">
    ...
    <div style="border-top: 1px solid var(--border); padding-top: 1rem;">
        <p style="font-size: 0.875rem; color: var(--text-secondary);">
            <strong>Localisation:</strong> <?= e($annonce['location'] ?? 'Non spécifiée') ?>
        </p>
        <p style="font-size: 0.875rem; color: var(--text-secondary); margin-top: 0.5rem;">
            <strong>Vues:</strong> <?= (int)$annonce['views'] ?>
        </p>
    </div>
</div>

            <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                <?php if (isset($_SESSION['utilisateur_id']) && (int)$_SESSION['utilisateur_id'] !== (int)$annonce['user_id']): ?>
                    <a href="<?= url('/pages/messages/envoyer.php?annonce_id=' . $annonce['id'] . '&destinataire_id=' . $annonce['user_id']) ?>" class="btn btn-primary">💬 Contacter le vendeur</a>
                <?php endif; ?>
                
                <?php if (isset($_SESSION['utilisateur_id'])): ?>
                    <form method="POST" action="<?= url('/pages/favoris/toggle.php') ?>" style="display: inline;">
                        <?= csrf_field() ?>
                ...
                <?php if (isset($_SESSION['utilisateur_id']) && ((int)$_SESSION['utilisateur_id'] === (int)$annonce['user_id'] || ($_SESSION['utilisateur_role'] ?? '') === 'admin')): ?>
                    <a href="<?= url('/pages/annonces/modifier.php?id=' . $annonce['id']) ?>" class="btn btn-secondary">✏️ Modifier</a>
                    <form method="POST" action="<?= url('/pages/annonces/supprimer.php') ?>" style="display: inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette annonce ?');">
                        <?= csrf_field() ?>
